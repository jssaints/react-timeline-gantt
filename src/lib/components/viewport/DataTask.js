...
export default class DataTask extends Component {
    ...
    calculateStyle() {
        ...
        const topValue = Math.floor((this.props.height - 26) / 2);
        if (this.state.dragging) {
            return { ...configStyle, backgroundColor: backgroundColor, left: this.state.left, width: this.state.width, height: 26, top: topValue, borderRadius: 3, padding: 4, zIndex: 1 }
        } else {
            return { ...configStyle, backgroundColor, left: this.props.left, width: this.props.width, height: 26, top: topValue, borderRadius: 3, padding: 4, zIndex: 1 }
        }

    }
    render() {
        let style = this.calculateStyle();
        const { enableLinker, itemNumber } = this.props;
        let topTodayLineStyle;
        if (this.props.borderLeft) {
            this.props.borderLeft.map((borderLeft) => {
                if (borderLeft.props.id !== undefined && itemNumber === 0) {
                    topTodayLineStyle = borderLeft.props.style;
                }
            });
        }
        return (
            <React.Fragment>
                {this.props.borderLeft && this.props.borderLeft.map((borderLeft, index) => (
                    <React.Fragment key={`borderLeft-${index}`}>
                        {borderLeft}
                    </React.Fragment>
                ))}
                {topTodayLineStyle !== "" && (
                    <div className="timeline-today" style={topTodayLineStyle} />
                )}
                <div
                    ...
                    <div style={{ overflow: 'hidden' }} onDoubleClick={(e) => { this.props.onClickItem(this.props.item) }} className="text-truncate">
                        {Config.values.dataViewPort.task.showLabel ? this.props.item.name : ""}
                    </div>
                    <div className="timeLine-main-data-task-side"
                        style={{ top: 0, left: style.width - 6, height: style.height }}
