...
export class HeaderItem extends PureComponent {
    ...
    render() {
        return (
            <div className="timeline-header-item" style={{ display: 'flex', justifyContent: 'center', alignItems: 'center', borderLeft: 'solid 1px white', position: 'absolute', height: 30, left: this.props.left, width: this.props.width }}>
                <div>
                    {this.props.label}
                </div>
            </div>)
...
export default class Header extends PureComponent {
    ...
    getIntlFormat(mode, position) {
        switch (mode) {
            ...
            case 'week':
                if (position == 'top')
                    return { weekday: 'short', year: 'numeric', month: 'long' }
                else
    ...
    renderTime = (left, width, mode, key) => {
        ...
        return <div key={key} style={{ position: 'absolute', height: 30, left: left, width: width }}> {result}</div>;
    }
    ...
    renderHeaderRows = (top, middle, bottom) => {
        ...
        for (let i = start - BUFFER_DAYS; i < end + BUFFER_DAYS; i++) {
            ...
            const lng = (this.props.locale)? this.props.locale :  "en";
            if (currentTop != Intl.DateTimeFormat(lng, this.getIntlFormat(top, 'top')).format(currentDate)) {
                currentTop = Intl.DateTimeFormat(lng, this.getIntlFormat(top, 'top')).format(currentDate);
