...
class TimeLine extends Component {
  ...
  horizontalChange = newScrollLeft => {
    ...
    new_endRow =
      new_startRow + this.state.numVisibleRows >= this.state.data.length
  ...
  checkNeeeData = () => {
    if (this.props.data != this.state.data) {
      ...
      if (this.props.groupByID) {
        const groupedData = DataGroupBy(this.props);
        const newData = [];
        if (Object.keys(groupedData).length > 0) {
          Object.keys(groupedData).map(function (key, index) {
            groupedData[key].map(function (item) {
              let newItem = Object.assign({}, item);
              newItem.groupBy_index = index;
      ...
      <div className="timeLine">
        <div className="timeLine-side-main" style={this.state.sideStyle}>
          <TaskList
            ...
            groupByName={this.props.groupByName}
